...
- [x] Implement `DefaultRanker::rank`: compute `native_pixels` from the accepted set, sort
      via `compare_configs`, then call `record_preferences` on each config.

### Tests (inline `#[cfg(test)]` in `src/ranker/mod.rs`)

- [x] `dsc_penalized_ranked_lower` — DSC config follows non-DSC under `penalize_dsc`.
- [x] `dsc_not_penalized_under_performance` — DSC config is not demoted under `BEST_PERFORMANCE`.
- [x] `native_resolution_ranked_first` — higher-res mode ranks first under `prefer_native_resolution`.
- [x] `color_fidelity_prefers_higher_depth` — 10-bit before 8-bit under `BEST_QUALITY`.
- [x] `color_fidelity_prefers_rgb_over_ycbcr444` — `Rgb444` before `YCbCr444` at equal depth.
- [x] `color_format_quality_full_ordering` — `Rgb444 > YCbCr444 > YCbCr422 > YCbCr420` under
      `BEST_QUALITY`.
- [x] `performance_prefers_high_refresh` — 120 Hz before 60 Hz under `BEST_PERFORMANCE` even at
      lower bit depth.
- [x] `power_saving_prefers_low_refresh` — 60 Hz before 120 Hz under `POWER_SAVING`.
- [x] `power_saving_prefers_low_depth` — 8-bit before 10-bit under `POWER_SAVING`.
- [x] `power_saving_prefers_simpler_format` — `YCbCr420` before `Rgb444` under `POWER_SAVING`.
- [x] `progressive_before_interlaced` — progressive mode ranks above the same mode interlaced.
- [x] `lower_frl_rate_tiebreak` — `Rate3Gbps3Lanes` before `Rate6Gbps3Lanes` for otherwise
      identical configs.
- [x] `empty_input_returns_empty` — returns empty vec without panic.
- [x] `single_input_returns_unchanged` — single config is returned as-is.
- [x] `trace_records_dsc_penalty` — `PreferenceApplied` step present on a DSC config when
      `penalize_dsc` is set.
- [x] `trace_records_native_resolution` — `PreferenceApplied` step present on the native-res
